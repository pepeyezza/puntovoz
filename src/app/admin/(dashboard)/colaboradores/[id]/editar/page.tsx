...
import { ESPECIALIDADES_COLABORADOR } from "@/lib/categorias";
    ...
    <div>
      ...
      <form action={updateColaboradorProfile.bind(null, colaborador.id)} className="mt-8 max-w-lg space-y-5">
        <div>
          ...
          <input name="name" type="text" required defaultValue={colaborador.name} className="mt-2 w-full rounded-xl border border-principal/15 bg-secundario px-4 py-3 text-sm outline-none focus-visible:border-acento" />
        </div>

        <div>
          <label className="text-sm font-medium">Especialidad</label>
          <select name="especialidad" defaultValue={(colaborador as any).especialidad ?? ""} className="mt-2 w-full rounded-xl border border-principal/15 bg-secundario px-4 py-3 text-sm outline-none focus-visible:border-acento">
            <option value="">— Sin especialidad —</option>
            {ESPECIALIDADES_COLABORADOR.map((e) => (
        ...
        <div>
          ...
          <textarea name="bio" rows={3} defaultValue={colaborador.bio ?? ""} className="mt-2 w-full rounded-xl border border-principal/15 bg-secundario px-4 py-3 text-sm outline-none focus-visible:border-acento" />
        </div>
