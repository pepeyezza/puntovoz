    ...
    <div>
      ...
      <form action={createServicio} className="mt-8 max-w-xl space-y-5">
        ...
        <div>
          ...
          <input name="area" type="text" placeholder="Ej: Biologia molecular, Quimica..." className="mt-2 w-full rounded-xl border border-principal/15 bg-secundario px-4 py-3 text-sm outline-none focus-visible:border-acento" />
        </div>
        <div>
          <label className="text-sm font-medium">Descripcion</label>
          <div className="mt-2">
            <RichTextEditor name="descripcion" placeholder="Descripcion del servicio..." />
          </div>
        </div>
        <div>
          <label className="text-sm font-medium">Contacto para este servicio</label>
          <input name="contacto" type="text" placeholder="Email o telefono de contacto" className="mt-2 w-full rounded-xl border border-principal/15 bg-secundario px-4 py-3 text-sm outline-none focus-visible:border-acento" />
        </div>
        <ImageUploadField name="imagen" label="Imagen del servicio (opcional)" />
        <div className="flex gap-3 pt-2">
          <button type="submit" className="rounded-lg bg-principal px-6 py-3 text-sm font-semibold text-secundario">Guardar</button>
          <a href={`/admin/observatorio/investigacion/${inst.id}`} className="rounded-lg border border-principal/15 px-6 py-3 text-sm font-medium hover:border-acento hover:text-acento">Cancelar</a>
        </div>
      </form>
    </div>
  );
