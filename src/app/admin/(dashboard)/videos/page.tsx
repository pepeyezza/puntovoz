    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-principal/60">Videos embebidos desde YouTube. El campo "Orden" define el orden de aparicion (1 = primero).</p>
        </div>
        <Link href="/admin/videos/nuevo" className="rounded-lg bg-principal px-5 py-2.5 text-sm font-semibold text-secundario hover:-translate-y-0.5">
          + Nuevo video
        </Link>
      </div>
      <div className="mt-8 overflow-hidden rounded-2xl border border-principal/10">
        <table className="w-full text-sm">
          ...
          <tbody>
            {videos.length === 0 ? (
              <tr><td colSpan={5} className="px-5 py-8 text-center text-principal/50">Todavia no hay videos cargados.</td></tr>
            ) : (
              videos.map((v) => (
                <tr key={v.id} className="border-t border-principal/10">
                  ...
                  <td className="px-5 py-3">
                    <span className={`rounded-lg px-3 py-1 text-xs font-semibold ${v.status === "PUBLISHED" ? "bg-joven/20 text-principal" : "bg-principal/10 text-principal/60"}`}>
