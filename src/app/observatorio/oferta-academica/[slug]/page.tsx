...
async function getInst(slug: string) {
  try {
    return await prisma.institucionAcademica.findUnique({
      where: { slug },
      include: { carreras: { orderBy: { nombre: "asc" } } },
    });
  } catch { return null; }
}
    ...
    <section className="mx-auto max-w-editorial px-5 py-16 lg:px-8">
      ...
      <div className="mt-12">
        ...
        {areas.length > 1 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {areas.map((area) => (
              <a key={area}
                href={area === "Todas" ? `/observatorio/oferta-academica/${inst.slug}` : `/observatorio/oferta-academica/${inst.slug}?area=${encodeURIComponent(area)}`}
                className={`rounded-full border px-4 py-1.5 text-xs font-medium transition-colors ${areaActiva === area ? "border-acento bg-acento text-secundario" : "border-principal/15 hover:border-acento hover:text-acento"}`}>
                {area}
              </a>
            ))}
          </div>
      ...
      {carreras.length === 0 ? (
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          {carreras.map((c) => (
            <div key={c.id} className="flex flex-col rounded-2xl border border-principal/10 overflow-hidden">
              {(c as any).imagen && (
                <div className="relative aspect-video w-full">
                  <Image src={(c as any).imagen} alt={c.nombre} fill className="object-cover" />
                </div>
              )}
              <div className="p-6">
                ...
                {c.descripcion && (
                  <div className="mt-3 prose-voz text-sm text-principal/60" dangerouslySetInnerHTML={{ __html: c.descripcion }} />
                )}
